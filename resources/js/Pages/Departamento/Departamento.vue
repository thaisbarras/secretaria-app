        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <Link :href="route('departamento.create')">
                    ...
                    <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
                        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
                            ...
                            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                                <tr v-for="dpto in dptos" :key="dpto.id" class="hover:bg-gray-50">
                                <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                    <div class="relative h-10 w-10">
                                        {{ dpto.id }}
                                    </div>
                                </th>
                                <td class="px-6 py-4">{{ dpto.nome }}</td>
                                <td class="px-6 py-4">{{ dpto.sigla }}</td>
                                <!-- Editar e Excluir -->

                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-4">
                                        <Link 
                                            :href="route('departamento.edit', dpto.id)"
...
const destroy = (dptos) => {
        if(confirm('Tem certeza que deseja excluir?')) {
            router.delete(route('departamento.destroy', dptos))
        }
    }   

    export default {
        name: 'PageDepartamento',
